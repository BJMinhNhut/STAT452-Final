...
```
```{r}
model = update(model_full, . ~ . - Length2)
car::vif(model)
```

```{r}
model = update(model, . ~ . - Length3)
car::vif(model)
```
...
```
```{r}
model = update(model, . ~ . - Length1)
car::vif(model)
```

```{r}
model_bw <- step(model, data = train_set, direction = "both")
```
